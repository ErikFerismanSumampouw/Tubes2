...
if (isset($_GET['delete_serial'])) {
    ...
    try {
       ...
       //DELETE SENSOR
        $sql_delete_sensor ="DELETE 
                            FROM Sensor 
                            WHERE serialNumSen = ?";
        $stmt_delete_sensor = sqlsrv_query($conn, $sql_delete_sensor, array($serialToDelete));

        if ($stmt_delete_sensor === false) {
            throw new Exception(print_r(sqlsrv_errors(), true));
        }

        //DELETE SENSOR/AKTUATOR DARI TABEL PERANGKATIOT
        $sql_delete_perangkat = "DELETE 
                                FROM PerangkatIOT 
                                WHERE serialNum = ?";
